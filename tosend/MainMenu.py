...
def run_script(script_name):
    subprocess.run(["python3", os.path.join(os.path.dirname(os.path.abspath(__file__)), script_name)])

def run_menu312():
    run_script("menu312.py")

def run_menu313():
    run_script("module15/menu313.py")

def run_script_with_bash(script_name):
    subprocess.run(["bash", os.path.join(os.path.dirname(os.path.abspath(__file__)), script_name)])

def run_322a():
    run_script_with_bash("run322a.sh")

def run_322b():
    run_script_with_bash("run322b.sh")

def run_323():
    run_script("country_news_range.py")

def run_324():
    run_script("country_range_time.py")

def run_325():
    run_script("Jaccard.py")
...
if __name__ == "__main__":
    print("----Wait while we are extracting the required file----")
    subprocess.run(["python3", os.path.join(os.path.dirname(os.path.abspath(__file__)), "extractNR.py")])
    run_script("Run_australia.py")
    run_script("Run_england.py")
    run_script("Run_india.py")
    run_script("Run_malaysia.py")
    run_script("Run_singapore.py")
    run_script("DataConvertor.py")
    main()
